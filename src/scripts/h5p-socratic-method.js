...
import SocraticMethodContent from './h5p-socratic-method-content.js';
import Dictionary from '@services/dictionary.js';
import Util from '@services/util.js';

/** Class representing SocraticMethod */
export default class SocraticMethod extends H5P.Question {
  /**
   * @class
  ...
  constructor(params, contentId, extras = {}) {
    super('socratic-method'); // CSS class selector for content's iframe: h5p-socratic-method
    ...
    this.extras = Util.extend({
      metadata: {
        title: 'SocraticMethod',
      },
      previousState: SocraticMethod.getPreviousStateLocal(
  ...
  registerDomElements() {
    ...
    this.params.behaviour.showNotesOnStartup = this.params.behaviour.showNotesOnStartup &&
      document.querySelector('.h5p-container').offsetWidth >= SocraticMethod.MIN_WIDTH_FOR_DUALVIEW;

    this.content = new SocraticMethodContent({
      initialMsg: this.params.initialMsg,
      courseContent: this.params.courseContent,
  ...
  getTitle() {
    ...
    raw = raw || SocraticMethod.DEFAULT_DESCRIPTION;
   ...
   */
  getDescription() {
    return this.params.taskDescription || SocraticMethod.DEFAULT_DESCRIPTION;
  }
    ...
    try {
      if (window.localStorage) {
        ...
        window.localStorage.setItem(
          `${SocraticMethod.DEFAULT_DESCRIPTION}-${id}`,
...
/** @constant {string} */
SocraticMethod.DEFAULT_DESCRIPTION = 'SocraticMethod';
